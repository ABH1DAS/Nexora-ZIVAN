...
export default function HospitalInfoPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Hero profile card */}
      <div className="rounded-[2rem] border border-border bg-gradient-to-br from-white via-[#f3faf8] to-[#e8f6fb] p-6 shadow-sm sm:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-primary text-white shadow-xs">
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="rounded-xl border border-primary/20 bg-primary-soft px-3 py-1 text-xs font-bold text-primary shadow-2xs">
                  {hospitalData.open ? "Open 24/7" : "Currently Closed"}
                </span>
                <span className="rounded-xl border border-border bg-white px-3 py-1 text-xs font-semibold text-muted capitalize shadow-2xs">
                  {hospitalData.type}
                </span>
                <span className="rounded-xl border border-accent/20 bg-accent-soft px-3 py-1 text-xs font-semibold text-accent shadow-2xs">
                  NABH Accredited · Demo
                </span>
              </div>
            </div>
          </div>
          <div className="flex flex-col gap-2.5 rounded-[1.25rem] border border-border bg-white p-4 text-sm text-muted shadow-2xs">
      ...
      <section>
        ...
        <div className="flex flex-wrap gap-2.5">
          {services.map((svc) => (
            <span
              key={svc}
              className="rounded-2xl border border-border bg-white px-4 py-2 text-sm font-semibold text-foreground shadow-2xs hover:border-primary/30 hover:bg-primary-soft hover:text-primary hover:-translate-y-0.5 transition-all duration-200"
            >
              {svc}
            </span>
          ))}
