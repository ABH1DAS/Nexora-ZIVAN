...
export function IncidentReportModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl overflow-hidden rounded-[2rem] border border-border bg-white shadow-xl flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-border bg-[#f7fbfa] px-6 py-4 text-foreground print:hidden">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-primary text-white shadow-2xs">
              <FileText className="h-5 w-5" />
            </div>
            <div>
              <h3 className="font-display text-lg font-bold text-foreground">Emergency Incident Summary Report</h3>
              <p className="text-xs text-muted">
                Official Clinical Dispatch Record · ID: <code className="font-mono text-primary font-bold">{request.id}</code>
              </p>
            </div>
          </div>
          <button
        ...
        <div className="overflow-y-auto p-8 space-y-6 bg-white text-foreground print:p-0">
          ...
          <div className="flex items-start justify-between border-b border-border pb-5">
            ...
            <div className="text-right">
              <span className="inline-block rounded-full border border-border bg-slate-50 px-3 py-1 text-xs font-mono font-semibold text-slate-700">
                CASE: {request.id}
              </span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="rounded-xl border border-rose-200 bg-rose-50/70 p-3 shadow-2xs">
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2 text-center">
              <div className="rounded-xl border border-border bg-[#f7fbfa] p-2.5 shadow-2xs">
          ...
          <div className="space-y-3">
            <div className="rounded-xl border border-border bg-[#f7fbfa] p-3.5 text-xs space-y-1.5 shadow-2xs">
            ...
            {request.handoverNotes && (
              <div className="rounded-xl border border-primary/20 bg-primary-soft/50 p-3.5 text-xs space-y-1.5 shadow-2xs">
        ...
        {/* Modal Footer */}
        <div className="flex items-center justify-between border-t border-border bg-[#f7fbfa] px-6 py-4 print:hidden">
          <Button variant="outline" size="sm" onClick={onClose}>
            Close
          </Button>
          <Button variant="primary" size="sm" onClick={handlePrint}>
            <Printer className="h-4 w-4" />
            Print Official Case Summary
          </Button>
        </div>
      </div>
    </div>
